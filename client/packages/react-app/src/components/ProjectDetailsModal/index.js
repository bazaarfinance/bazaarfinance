      ...
      <DetailsModal>
        ...
        <DetailsRow>
          ...
          <DetailsColumn>
            ...
            <Info>
              <a href="https://kovan.etherscan.io/address/{recipientAddress}">
                {truncate(recipientAddress, 6, 4, '...')}
              </a>
            </Info>
            <PoweredByContainer>
              <Icon src={aDaiLogo} alt="aDai" />
              <Info id="poweredBy">Powered by</Info>
        ...
        <DetailsRow>
          ...
          <DetailsColumn id="projectStatus">
            <DetailsRow>
              <InfoTitle>Raised/Earned Ratio</InfoTitle>
            </DetailsRow>
            <ProgressBar>
              <Line percent="50" strokeWidth="5" strokeColor="#F9BBBB" />
            </ProgressBar>
            <DetailsRow>
              <InfoTitle>Interest Rate: 10% APY</InfoTitle>
            </DetailsRow>
          </DetailsColumn>
        </DetailsRow>
        { daiAllowance.toString() !== '0' || daiApproved ?
          <InputRow>
            <Input
              ref={inputRef}
              type="number"
        ...
        <ButtonsRow>
          ...
          >
            {daiAllowance.toString() !== '0' || daiApproved ? 'Deposit' : 'Approve Token'}
          </Button>
          {bTokenBalance > 0 ? (
            <Button
              onClick={async () => {
                try {
                  if (bTokenAllowance.toString() !== '0' || bTokenApproved) {
                    ...
                    let estimatedGas = await bToken.estimateGas.approve(
                      addresses.vault,
                      MaxUint256
                    );
            ...
            >
              {bTokenAllowance.toString() !== '0' || bTokenApproved
                ? 'Withdraw All'
                : 'Approve bToken'}
            </Button>
          ) : null}
